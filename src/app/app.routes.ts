...
const canMatchMonth: CanMatchFn = (route: Route, segments: UrlSegment[]) => {
  const urlMonth = [segments[0].path, segments[1].path];
  const store = inject(Store);
  const isValidMonth =
    isMonth(urlMonth) &&
    store.months.some((month) => compareMonths(month, urlMonth));
  if (isValidMonth) store.currentMonth = urlMonth;
  return isValidMonth;
};

const canMatchNoMonth: CanMatchFn = (route: Route, segments: UrlSegment[]) => {
  const store = inject(Store);
